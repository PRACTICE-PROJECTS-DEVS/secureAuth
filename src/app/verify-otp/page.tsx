...
export default function VerifyOtpPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // await api.post('/auth/verify-otp', { userId: getUserId(), otp: code });
      await new Promise((r) => setTimeout(r, 1000));
      const mockValid = code === '123456'; // demo only

      if (mockValid) {
        addToast('OTP verified successfully!', 'success');
        setTimeout(() => router.push('/verify-biometric'), 1000);
      } else {
    ...
    <>
      ...
      <AuthCard>
        ...
        <div className="text-center mb-8 animate-fade-up">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-blue-dim border border-blue-accent/20 mb-4 relative">
          ...
          <p className="text-muted text-sm font-mono mt-1">One-Time Password sent to</p>
          <p className="text-[#e8eaf6] text-sm font-mono font-semibold mt-0.5">{maskedEmail}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-7">
          ...
          <div className="animate-fade-up delay-100">
            <label className="block text-xs font-mono font-medium text-muted uppercase tracking-widest mb-4 text-center">
              Enter 6-Digit Code
            </label>
          ...
          <div className="flex items-center justify-center gap-2 animate-fade-up delay-200">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4a4d6a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
            </svg>
            <span className="text-xs text-muted font-mono">Code expires in 10 minutes</span>
          </div>
          ...
          <button
            ...
            className="
              w-full py-3.5 rounded-xl font-bold text-sm bg-blue-accent text-bg
          ...
          <div className="text-center animate-fade-up delay-400">
            <p className="text-xs text-muted font-mono mb-2">Didn't receive the code?</p>
            <button
              type="button"
              ...
              className="text-sm font-semibold transition-all duration-200 disabled:cursor-not-allowed flex items-center gap-1.5 mx-auto
                disabled:text-muted text-blue-accent hover:text-blue-accent/80"
            >
              {resendLoading ? (
                <>
        ...
        {/* Footer */}
        <div className="mt-6 pt-5 border-t border-border animate-fade-up delay-500">
          <button
        ...
        {/* Demo hint */}
        <div className="mt-4 p-3 rounded-lg bg-blue-dim border border-blue-accent/20">
          <p className="text-xs text-blue-accent font-mono text-center">
